...
vector<char> generar_randoms() { //ejercicio 8 //funcion para retornar un vector con 4 letras al azar
	vector<char> randoms;
	srand(time(0));
	for (int i = 0; i < 4; i++) {
		char convertidor = 'a' + rand() % 26;
		randoms.push_back(convertidor);
	}
	return randoms;
}
...
int main()
{ 
	vector<char> adivinar = generar_randoms(); //llenar adivinar con 4 letras al azar
	vector<char> jugador;
	char letra;
	int bulls = 0;
	...
	while (bulls != 4) {
	    int cows = 0;
		cout << "Bulls and cows, escribe 4 letras" << endl;
		for (int i = 0; i < adivinar.size(); i++) { // input del jugador
			cin >> letra;
			jugador.push_back(letra);
		}

		for (int x = 0; x < jugador.size(); x++) {
			if (jugador[x] == adivinar[x]) //chequeo de bulls
				bulls++;
			for (int y = 3; y > 0; y--) {
				if (jugador[x] != adivinar[x] && jugador[x] == adivinar[y])
					cows++; //si no es un bull y es lo mismo que en otro de los 3 restantes entonces es un cow
			}
		}
		cout << "Bulls = " << bulls << " Cows = " << cows << endl;

		if (bulls == 4) { //condicion de victoria y escape del ciclo
			cout << "Felicidades, 4 Bulls" << endl;
			break;
		}

		bulls = 0;   //reinicia el valor de bulls
		cows = 0;   //reinicia el valor de cows
		jugador.clear(); // limpia el vector del jugador 
	}
	keep_window_open();
	return 0;
